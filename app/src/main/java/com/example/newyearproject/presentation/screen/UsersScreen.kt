...
import com.example.newyearproject.presentation.details.DetailsScreenUserContent
...
fun UsersScreen(
    ...
    LaunchedEffect(viewModel.effect) {
        viewModel.effect.collectLatest { effect ->
            when (effect) {
                is AllEffect.OpenDetails_Users -> {
                    navigator.navigate(DetailsScreenUserContent(user = effect.item))
                }
            }
        }
    }
